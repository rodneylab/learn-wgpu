...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // This makes sure that they are perpendicular to each other and the normal of the surface.
    ...
    let diffuse_colour = light.color * diffuse_strength;

    let specular_strength = pow(max(dot(world_normal, half_dir), 0.0), 32.0);
    let specular_colour = specular_strength * light.color;

    let world_reflect = reflect(-view_dir, world_normal);
    let reflection = textureSample(env_map, env_sampler, world_reflect).rgb;
    let shininess = 0.1;

    let result = (diffuse_colour + specular_colour) * object_colour.xyz + reflection * shininess;
