...
impl<'a> Texture<'a> {
    ...
    pub fn prepare_buffer_rgba(&self, device: &wgpu::Device) -> buffer::RawBuffer<[f32; 4]> {
        ...
        let buffer_size = num_pixels * u32::try_from(mem::size_of::<[f32; 4]>()).unwrap();
        let buffer_usage = wgpu::BufferUsages::COPY_DST | wgpu::BufferUsages::MAP_READ;
        let buffer_desc = wgpu::BufferDescriptor {
            size: wgpu::BufferAddress::from(buffer_size),
            usage: buffer_usage,
            label: None,
        ...
        buffer::RawBuffer::from_parts(buffer, data, buffer_usage)
    }
}
